...
export default class ParcelaController {
  ...
  static async listarPorStatus(req: Request, res: Response) {
    try {
      ...
      if (id_cliente) {
        parcelas = await Parcela.listarPorCliente(id_cliente);
      } else {
        const query = `
          SELECT p.*
          FROM Parcela p
      ...
      if (status === 'pagas') {
        ...
        filtradas = parcelas.filter(p => p.status_parcela === 'pendente' || p.status_parcela === 'atrasado');
      } else if (status === 'atrasadas') {
        filtradas = parcelas.filter(p => p.status_parcela === 'atrasado');
      }
